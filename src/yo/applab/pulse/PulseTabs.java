...
public class PulseTabs extends TabActivity {
	 ...
	 * 
	 * Responsible for getting and saving a few global settings, and setting up
	 * the UI: getting a list of tabs from the dataCollector, and calling
	 * dataCollector to load their contents.
	 * 
	 * @param savedInstanceState
	 *            Saved state from the last run of the application. We are not
	 *            using this yet, but we may do so in order to save the contents
	 *            of the tabs between application runs.
	 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Get saved tab data incase there was a configuration (orientation)
	...
	/**
	 * Displays a progress dialog and then calls the dataCollector to refresh
	...
	/**
	 * Populates tabs with data that has already been retrieved in the
	 * background by the dataCollector
	 */
	private void updateTabs() {
		for (int i = 0; i < tabList.size(); i++) {
			...
			if (tabInfo.supportsRefresh) { // short term hack to skip support
				...
				String tabData = tabInfo.storedTabData == null ? Global.errorHtml
						: tabInfo.storedTabData;
	...
	/**
	 * Handles responses from the data collection layer and updates the tab
	 * contents
	 */
	...
	/**
	 * Called when the device orientaion changes to save tab data
	 */
	@Override
